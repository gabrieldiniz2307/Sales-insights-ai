"""
Agente LangChain para processamento de perguntas sobre vendas - VERSÃO CORRIGIDA
...
class SalesLangChainAgent:
    ...
    def _safe_get(self, item: dict, key: str, default=None):
    ...
    def _format_response_from_data(self, question: str, query_result: Dict[str, Any]) -> str:
        ...
        if "produto mais vendido" in question_lower or "top produto" in question_lower:
            ...
            for i, item in enumerate(data[:5], 1):
                produto_nome = self._safe_get(item, 'produto_nome', 'N/A')
                sku = self._safe_get(item, 'sku', 'N/A')
                categoria = self._safe_get(item, 'categoria', 'N/A')
                total_quantidade = self._safe_get(item, 'total_quantidade', 0)
                total_receita = self._safe_get(item, 'total_receita', 0.0)
                total_pedidos = self._safe_get(item, 'total_pedidos', 0)
                
                response += f"{i}. **{produto_nome}** (SKU: {sku})\n"
                response += f"   • Categoria: {categoria}\n"
                response += f"   • Quantidade vendida: {total_quantidade} unidades\n"
                response += f"   • Receita: R$ {total_receita:.2f}\n"
                response += f"   • Total de pedidos: {total_pedidos}\n\n"
            ...
            item = data[0]
            total_vendas = self._safe_get(item, 'total_vendas', 0)
            receita_total = self._safe_get(item, 'receita_total', 0.0)
            total_produtos = self._safe_get(item, 'total_produtos', 0)
            total_clientes = self._safe_get(item, 'total_clientes', 0)
            ticket_medio = self._safe_get(item, 'ticket_medio', 0.0)
            
            response = f"""📊 **Resumo Completo das Vendas:**

• **Total de vendas:** {total_vendas} transações
...
💡 *Análise baseada em consulta direta ao banco de dados de vendas*"""
            ...
            for i, item in enumerate(data[:5], 1):
                produto_nome = self._safe_get(item, 'produto_nome', 'N/A')
                quantidade_vendida = self._safe_get(item, 'quantidade_vendida', 0)
                receita = self._safe_get(item, 'receita', 0.0)
                
                response += f"{i}. **{produto_nome}**\n"
                response += f"   • Quantidade: {quantidade_vendida} unidades\n"
                response += f"   • Receita: R$ {receita:.2f}\n\n"
            ...
            if len(data) == 1 and 'total_vendas' in data[0]:
                ...
                total_vendas = self._safe_get(item, 'total_vendas', 0)
                receita_total = self._safe_get(item, 'receita_total', 0.0)
                response = f"📈 **Análise dos Dados:** Encontrei {total_vendas} vendas com receita total de R$ {receita_total:.2f} no período analisado."
            else:
                response = f"📊 **Resultado da Consulta:** Encontrei {len(data)} registros relevantes para sua pergunta. "
                if data:
                    first_item = data[0]
                    first_value = list(first_item.values())[0] if first_item and first_item.values() else 'N/A'
                    response += f"Primeiro resultado: {first_value}"
...
*Esta limitação força o uso de RAG (busca direta no banco de dados)*""",
            ...
            method_info += f"\n📊 **Query executada:** {len(query_result.get('query_executed', '') or '')} caracteres"
